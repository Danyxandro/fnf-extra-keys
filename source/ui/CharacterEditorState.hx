...
class CharacterEditorState extends MusicBeatState
{
	...
	var isPlayer:Bool;
	var savePos:Array<Float>=[0.0,0.0,0.0,0.0];
	var saveCam:Array<Float>=[0.0,0.0,0.0,0.0];
	var healthBar:FlxBar;
	var tween:FlxTween;
	var savedX:Int;

	public function new(daAnim:String = 'spooky', goToPlayState:Bool = true, asPlayer:Bool = false)
	{
		super();
		this.daAnim = daAnim;
		this.goToPlayState = goToPlayState;
		this.isPlayer = asPlayer;
	}
	...
	override function create()
	{
		...
		{
			//onPixelBG = !onPixelBG;
			showSilueta = !showSilueta;
			changeBGbutton.text = showSilueta ? "Hide ref" : "Show ref";
			reloadBGs();
		});
		changeBGbutton.text = "Show ref";
		...
		if (sys.FileSystem.exists(Paths.json('healthBar'))){
			var datos:haxe.DynamicAccess<Dynamic> = cast haxe.Json.parse( lime.utils.Assets.getText( Paths.json('healthBar') ).trim() );
			for(key in datos.keys()){
				...
				colorsMap[key] = [rgb[0],rgb[1],rgb[2]];
			}
		}
		var color:FlxColor;
		if(colorsMap.exists(daAnim)){
			color = FlxColor.fromRGB(colorsMap.get(daAnim)[0], colorsMap.get(daAnim)[1], colorsMap.get(daAnim)[2]);
			healthBar.createFilledBar(color,color);
			healthColorStepperR.value = char.colorCode[0] = colorsMap.get(daAnim)[0];
			healthColorStepperG.value = char.colorCode[1] = colorsMap.get(daAnim)[1];
			healthColorStepperB.value = char.colorCode[2] = colorsMap.get(daAnim)[2];
		}else{
			color = FlxColor.fromRGB(char.colorCode[0], char.colorCode[1], char.colorCode[2]);
			healthBar.createFilledBar(color,color);
		}
		healthBar.updateBar();
	...
	private var OFFSET_Y:Float = 350;
	private var showSilueta:Bool = false;
	function reloadBGs() {
		...
		if(onPixelBG) {
			...
			var bgSky:FlxSprite = new FlxSprite(OFFSET_X - (playerXDifference / 2) - 300, 0 - playerYDifference);
			...
			var bgSchool:FlxSprite = new FlxSprite(repositionShit, -playerYDifference + 6);
			bgSchool.loadGraphic(Paths.image('weeb/weebSchool'),"week6");
			bgSchool.scrollFactor.set(0.6,0.9);
			bgLayer.add(bgSchool);
			bgSchool.antialiasing = false;

			var bgStreet:FlxSprite = new FlxSprite(repositionShit, -playerYDifference);
			...
			silueta = new FlxSprite(100 + OFFSET_X, OFFSET_Y);
			if(this.isPlayer){
				silueta.loadGraphic(Paths.image('editors/silhouetteBF',"shared"));
				silueta.offset.set(-6, 2);
			}else{
				silueta.loadGraphic(Paths.image('editors/silhouetteDad',"shared"));
				silueta.offset.set(-4, 1);
			}
			silueta.antialiasing = FlxG.save.data.antialiasing;
			silueta.active = false;
			silueta.visible = showSilueta;
			bgLayer.add(silueta);
		}
	}
	...
	function addSettingsUI() {
		...
		check_player.checked = char.isPlayer;//this.isPlayer;
		...
		charDropDown = new FlxUIDropDownMenuCustom(10, 30, FlxUIDropDownMenuCustom.makeStrIdLabelArray([''], true), function(character:String)
		{
			...
			loadChar(this.isPlayer);
			savePos=[char.posOffsets[0],char.posOffsets[1],char.posOffsets[0],char.posOffsets[1]];
			saveCam=[char.cameraPosition[0],char.cameraPosition[1],char.cameraPosition[0],char.cameraPosition[1]];
			if(char.camPlayerPosition != null){
				saveCam[0] = char.camPlayerPosition[0];
				saveCam[1] = char.camPlayerPosition[1];
			}
			if(char.playerPos != null){
				savePos[0] = char.playerPos[0];
				savePos[1] = char.playerPos[1];
			}
			updatePresence();
		...
		var templateCharacter:FlxButton = new FlxButton(140, 50, "Load Template", function()
		{
			...
			char.frames = ghostChar.frames = Paths.getSparrowAtlas('characters/DADDY_DEAREST', 'shared');
			imageInputText.text = "";
			for (character in characters)
			{
				...
				for (anim in character.animationsArray)
				{
					...
					character.animation.addByPrefix(anim.anim,anim.name,anim.fps);
				}
				...
				character.posOffsets = parsedJson.position;
				character.cameraPosition = parsedJson.camera_position;
				character.camPlayerPosition = parsedJson.camera_position;
				
				//character.imageFile = parsedJson.image;
				character.scale.set(parsedJson.scale,parsedJson.scale);
				character.antialiasing = !parsedJson.no_antialiasing;
	...
	var flyingStepper:FlxCustomStepper;
	...
	function addCharacterUI() {
		...
		flyingStepper = new FlxCustomStepper(reloadImage.x-30, reloadImage.y + 45,1,0,0);
		...
		positionXStepper = new FlxUINumericStepper(flipXCheckBox.x + 110, flipXCheckBox.y, 10, char.posOffsets[0], -9000, 9000, 0);
		positionYStepper = new FlxUINumericStepper(positionXStepper.x + 60, positionXStepper.y, 10, char.posOffsets[1], -9000, 9000, 0);
		savePos=[char.posOffsets[0],char.posOffsets[1],char.posOffsets[0],char.posOffsets[1]];
		if(char.playerPos != null){
			savePos[0] = char.playerPos[0];
			savePos[1] = char.playerPos[1];
		...
		positionCameraXStepper = new FlxUINumericStepper(positionXStepper.x, positionXStepper.y + 40, 10, 0, -9000, 9000, 0);
		positionCameraYStepper = new FlxUINumericStepper(positionYStepper.x, positionYStepper.y + 40, 10, 0, -9000, 9000, 0);
		saveCam=[char.cameraPosition[0],char.cameraPosition[1],char.cameraPosition[0],char.cameraPosition[1]];
		if(char.camPlayerPosition != null){
			saveCam[0] = char.camPlayerPosition[0];
			saveCam[1] = char.camPlayerPosition[1];
		...
		healthColorStepperR = new FlxUINumericStepper(singDurationStepper.x, saveCharacterButton.y, 20, char.colorCode[0], 0, 255, 0);
		healthColorStepperG = new FlxUINumericStepper(singDurationStepper.x + 65, saveCharacterButton.y, 20, char.colorCode[1], 0, 255, 0);
		healthColorStepperB = new FlxUINumericStepper(singDurationStepper.x + 130, saveCharacterButton.y, 20, char.colorCode[2], 0, 255, 0);

		tab_group.add(new FlxText(15, imageInputText.y - 18, 0, 'Character name:'));
		tab_group.add(new FlxText(15, healthIconInputText.y - 18, 0, 'Health icon name:'));
		tab_group.add(new FlxText(15, singDurationStepper.y - 18, 0, 'Sing Animation length:'));
		tab_group.add(new FlxText(15, scaleStepper.y - 18, 0, 'Scale:'));
		tab_group.add(new FlxText(flyingStepper.x, flyingStepper.y - 18, 0, 'Flying Offset:'));
		...
		var addUpdateButton:FlxButton = new FlxButton(70, animationIndicesInputText.y + 30, "Add/Update", function() {
			...
			for (anim in char.animationsArray) {
				if(animationInputText.text == anim.anim) {
					if(char.isPlayer)
						lastOffsets = anim.offsetsPlayer;
					else
		...
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			if(sender == healthIconInputText) {
				...
				if(colorsMap.exists(healthIconInputText.text)){
					trace("Found: "+healthIconInputText.text);
					/*healthColorStepperR.value = colorsMap.get(healthIconInputText.text)[0];
					healthColorStepperG.value = colorsMap.get(healthIconInputText.text)[1];
					healthColorStepperB.value = colorsMap.get(healthIconInputText.text)[2];*/
			...
			{
				char.posOffsets[0] = positionXStepper.value;
				char.x = char.posOffsets[0] + OFFSET_X + 100;
				if(char.isPlayer){
					savePos[0] = positionXStepper.value;
					if(char.playerPos == null)
						char.playerPos = [0,positionYStepper.value];
					char.playerPos[0] = positionXStepper.value;
				}else
					savePos[2] = positionXStepper.value;
				updatePointerPos();
			}
			...
			{
				char.posOffsets[1] = positionYStepper.value;
				char.y = char.posOffsets[1] + OFFSET_Y;
				if(char.isPlayer){
					savePos[1] = positionYStepper.value;
					if(char.playerPos == null)
						char.playerPos = [positionXStepper.value,0];
					char.playerPos[1] = positionYStepper.value;
				}else
					savePos[3] = positionYStepper.value;
				updatePointerPos();
			}
			else if(sender == positionCameraXStepper)
			{
				if(char.isPlayer){
					if(char.camPlayerPosition == null)
						char.camPlayerPosition = [0,positionCameraYStepper.value];
					char.camPlayerPosition[0] = positionCameraXStepper.value;
					saveCam[0] = positionCameraXStepper.value;
				}else{
					char.cameraPosition[0] = positionCameraXStepper.value;
					saveCam[2] = positionCameraXStepper.value;
				}
				updatePointerPos();
			}
			else if(sender == positionCameraYStepper)
			{
				if(char.isPlayer){
					if(char.camPlayerPosition == null)
						char.camPlayerPosition = [positionCameraXStepper.value,0];
					char.camPlayerPosition[1] = positionCameraYStepper.value;
					saveCam[1] = positionCameraYStepper.value;
				}else{
					char.cameraPosition[1] = positionCameraYStepper.value;
					saveCam[3] = positionCameraYStepper.value;
				}
				updatePointerPos();
			}
			else if(sender == healthColorStepperR)
			{
				char.colorCode[0] = Math.round(healthColorStepperR.value);
				//healthBarBG.color = FlxColor.fromRGB(char.colorCode[0], char.colorCode[1], char.colorCode[2]);
				resetHealthBarColor();
			}
			else if(sender == healthColorStepperG)
			{
				char.colorCode[1] = Math.round(healthColorStepperG.value);
				//healthBarBG.color = FlxColor.fromRGB(char.colorCode[0], char.colorCode[1], char.colorCode[2]);
				resetHealthBarColor();
			}
			else if(sender == healthColorStepperB)
			{
				char.colorCode[2] = Math.round(healthColorStepperB.value);
				//healthBarBG.color = FlxColor.fromRGB(char.colorCode[0], char.colorCode[1], char.colorCode[2]);
				resetHealthBarColor();
			}
			else if(sender == flyingStepper){
				char.flyingOffset = flyingStepper.value;
			}
		}
	}
	...
	function reloadCharacterImage() {
		var lastAnim:String = '';
		var routePNG:String = "assets/shared/images/characters/" + imageInputText.text + "/" + imageInputText.text + ".png";
		var routeXML:String = "assets/shared/images/characters/" + imageInputText.text + "/" + imageInputText.text + ".xml";
		if(char.animation.curAnim != null) {
	...
	function loadChar(isPlayable:Bool, blahBlahBlah:Bool = true) {
		...
		charLayer.clear();
		ghostChar = new Character(0, 0, daAnim, isPlayable);
		ghostChar.debugMode = true;
		ghostChar.alpha = 0.6;

		char = new Character(0, 0, daAnim, isPlayable);
		if(char.animationsArray[0] != null) {
			char.playAnim(char.animationsArray[0].anim, true);
		}
		char.debugMode = true;

		OFFSET_Y = isPlayable ? 350 : 0;

		charLayer.add(ghostChar);
		charLayer.add(char);

		if(isPlayable && char.playerPos != null)
			char.setPosition(char.playerPos[0] + OFFSET_X + 100, char.playerPos[1] + OFFSET_Y);
		else
			char.setPosition(char.posOffsets[0] + OFFSET_X + 100, char.posOffsets[1] + OFFSET_Y);
	...
	function updatePointerPos() {
		...
		if(!char.isPlayer) {
			...
			x -= 100 + char.camPlayerPosition[0];
			y -= 100 - char.camPlayerPosition[1];
			}
		}
	...
	function reloadCharacterOptions() {
		if(UI_characterbox != null) {
			imageInputText.text = ""+daAnim;
			healthIconInputText.text = daAnim;
			...
			if(char.isPlayer && char.playerPos != null){
				positionXStepper.value = char.playerPos[0];
				positionYStepper.value = char.playerPos[1];
			}else{
	...
	function reloadGhost() {
		...
		for (anim in char.animationsArray) {
			...
			if(char.isPlayer){
				if(anim.offsetsPlayer != null && anim.offsetsPlayer.length > 1) {
					ghostChar.addOffset(anim.anim, anim.offsetsPlayer[0], anim.offsetsPlayer[1]);
				}
			}else{
		...
		ghostChar.setGraphicSize(Std.int(ghostChar.width * char.scale.x));
	...
	function resetHealthBarColor(setSteppers:Bool = false) {
		...
		var color = FlxColor.fromRGB(char.colorCode[0], char.colorCode[1], char.colorCode[2]);
		healthBar.createFilledBar(color,color);
		healthBar.updateBar();
	}
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.ENTER) {
			trace("Save pos: "+savePos+"\nSave cam: "+saveCam);
			trace(char.animationsArray[curAnim]);
		}
		#end
		if(!charDropDown.dropPanel.visible) {
			...
			if(char.animationsArray.length > 0) {
				...
				if (FlxG.keys.justPressed.T)
				{
					...
					if(char.isPlayer)
						offset = char.animationsArray[curAnim].offsetsPlayer = [0, 0];
					else
						offset = char.animationsArray[curAnim].offsets = [0, 0];
					
					char.addOffset(char.animationsArray[curAnim].anim, offset[0], offset[1]);
					ghostChar.addOffset(char.animationsArray[curAnim].anim, offset[0], offset[1]);
					genBoyOffsets();
				}
				...
				for (i in 0...controlArray.length) {
					if(controlArray[i]) {
						...
						if(char.isPlayer){
							char.animationsArray[curAnim].offsetsPlayer[arrayVal] += negaMult * multiplier;
							offset = char.animationsArray[curAnim].offsetsPlayer;
						}else{
							char.animationsArray[curAnim].offsets[arrayVal] += negaMult * multiplier;
							offset = char.animationsArray[curAnim].offsets;
						}
						
						char.animOffsets.set(char.animationsArray[curAnim].anim,offset);
						//char.addOffset(char.animationsArray[curAnim].anim, offset[0], offset[1]);
						ghostChar.addOffset(char.animationsArray[curAnim].anim, offset[0], offset[1]);
	...
	function saveCharacter() {
		...
		var json = {
			...
			"playerPosition": [savePos[0],savePos[1]],
			"playerCameraPosition": [saveCam[0],saveCam[1]],
		
			"flip_x": char.originalFlipX,
			"no_antialiasing": !char.antialiasing,
			"healthbar_colors": char.colorCode,
			"flying_offset": char.flyingOffset
		};

		/*if(this.isPlayer){
			Reflect.setProperty(json, "playerPosition", [savePos[0],savePos[1]]);
			Reflect.setProperty(json, "playerCameraPosition", [saveCam[0],saveCam[1]]);
		}*/
