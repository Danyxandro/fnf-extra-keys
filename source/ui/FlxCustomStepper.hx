...
class FlxCustomStepper extends FlxUIGroup implements IFlxUIWidget implements IFlxUIClickable implements IHasParams
{
	...
	public var stepSizeShift:Float = 0;
	 ...
	 * @param	TextLenght			Optional text field width
	 * @param	shiftStepSize		How big is the step while pressing shift
	 ...
	 */
	public function new(X:Float = 0, Y:Float = 0, StepSize:Float = 1, DefaultValue:Float = 0, Min:Float = -999, Max:Float = 999, Decimals:Int = 0, textLenght:Int = 25,
			?shiftStepSize:Float, ?Stack:Int = STACK_HORIZONTAL, ?TextField:FlxText, ?ButtonPlus:FlxUITypedButton<FlxSprite>, ?ButtonMinus:FlxUITypedButton<FlxSprite>,
			IsPercent:Bool = false)
	{
		...
		if (TextField == null)
		{
			TextField = new FlxUIInputText(0, 0, textLenght);
		}
		TextField.x = 0;
		TextField.y = 0;
		...
		stepSize = StepSize;
		if(shiftStepSize == null){
			stepSizeShift = stepSize * 10;
		}else{
			stepSizeShift = shiftStepSize;
		}		
		decimals = Decimals;
		min = Min;
		max = Max;
